...
struct CRC32 {
    static private var table: [UInt32] = {
        (0..<256).map { i in
...
func pngChunk(type: String, data: Data) -> Data {
    var chunk = Data()
    ...
    var crc = CRC32.checksum(crcInput).bigEndian
    chunk.append(Data(bytes: &crc, count: 4))
    
    return chunk
}

func generateAPNG(frames: [Data], output: URL, frameDelay: UInt16 = 10, repeatCount: UInt32 = 0) {
    ...
    acTLData.append(repeatCount.bigEndian.data) // 循环次数，0 = 无限循环
...
func extractChunks(from pngData: Data, type: String) -> [Data] {
    var chunks: [Data] = []
    ...
    while cursor < pngData.count {
        ...
        let chunk = pngData[cursor..<(cursor+8+length+4)] // length + type + data + crc
        if chunkType == type {
            chunks.append(chunk)
        }
        cursor += 8 + length + 4
...
func extractIDATData(from pngData: Data) -> [Data] {
    var dataChunks: [Data] = []
...
func createfcTL(sequenceNumber: UInt32, widthHeightData: Data, delay: UInt16, isFirst: Bool) -> Data {
    ...
    data.append(UInt16(1000).bigEndian.data) // delay denominator → delay = numerator/denominator seconds
